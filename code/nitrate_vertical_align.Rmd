---
title: "Prepare nitrate data and vertically align to (pH) climatology"
...
bgc_data_nitrate <- full_join(bgc_metadata_select, bgc_data_nitrate) %>%
                          select(
                            file_id,
                            lat, 
                            lon, 
...
# ensure we have a depth, pres_adjusted and nitrate_adjusted for all rows in bgc_data_nitrate
bgc_data_nitrate <- bgc_data_nitrate %>%
                  filter(!is.na(depth) & !is.na(pres_adjusted) & !is.na(nitrate_adjusted))
...
for (i in 2:nrow(opt_profile_depth_range)) {
  ...
  #                         filter(!is.na(min_depth))
  bgc_data_nitrate <- bgc_data_nitrate %>% rows_update(bgc_apply_limits, by = "file_id")

}

# Find the gaps within the profiles
profile_gaps <- bgc_data_nitrate %>%
  select(
    file_id,
    depth
    ) %>%
...
for (i_gap in opt_gap_limit) {
  ...
  bgc_data_nitrate <- full_join(bgc_data_nitrate, profile_gaps_remove) %>%
                                  filter(is.na(gap)) %>%
                                  select(-c(gap))
}
...
target_depth_grid <-
  expand_grid(target_depth_range,
              bgc_data_nitrate_clean %>% distinct(file_id, lat, lon, date, profile_range))
...
bgc_data_nitrate_extended <- full_join(bgc_data_nitrate_clean, target_depth_grid) %>% 
...
bgc_data_nitrate_interpolated <-
  ...
  mutate(nitrate_spline = spline(target_depth, nitrate,
                                method = "natural",
                                xout = target_depth)$y) %>%
  ungroup()
...
bgc_data_nitrate_interpolated_clean <- 
  inner_join(target_depth_grid, bgc_data_nitrate_interpolated)
...
bgc_data_nitrate_interpolated_clean <- bgc_data_nitrate_interpolated_clean %>%
  mutate(year = year(date),
         month = month(date),
         .after = date)

# clean up working tables
rm(bgc_data_nitrate_extended, bgc_data_nitrate_interpolated)
...
rm(bgc_data_nitrate_interpolated_clean, ucsd_clim)
...
nitrate_overall_mean_1 %>% 
  ...
  geom_path(aes(x = nitrate_mean,
                y = depth))+
  geom_ribbon(aes(xmax = nitrate_mean + nitrate_sd,
  ...
  labs(title = paste0('Overall mean nitrate to ', max_depth_1, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_overall_mean_2 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Overall mean nitrate to ', max_depth_2, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_overall_mean_3 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Overall mean nitrate to ', max_depth_3, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_year_mean_1 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean nitrate to ', max_depth_1, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_year_mean_2 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean nitrate to ', max_depth_2, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_year_mean_3 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean nitrate to ', max_depth_3, 'm'), x=opt_measure_label, y='depth (m)')
...
nitrate_histogram %>% 
  ...
  labs(title = "nitrate profiles per year and profile range", x = "year", y = "profile count", fill = "profile range")
