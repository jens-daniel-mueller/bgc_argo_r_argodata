...
# opt_profile_range The depth that profiles must be valid to
# opt_depth_limit must also be set to correspond with opt_profile_range
# opt_profile_range = c(1, 2, 3) AND opt_depth_limit = 614 to get profiles of 614 m
# opt_profile_range = c(2, 3) AND opt_depth_limit = 1225 to get profiles of 1225 m
# opt_profile_range = c(3) AND opt_depth_limit = 1600 to get profiles of 1600 m
opt_profile_range = c(1, 2, 3)
opt_depth_limit = 614

# opt_exclude_profile this profile 
# During CORS analysis it was discovered that data associated with profiles of a single float (5905105) had unexpectedly
# extreme pH values. The location (approximately 286° E, 36° S) of this float indicated its proximity to the eastern
# south Pacific oxygen minimum zone off the western coast of Chile (Paulmier et al., 2006). Therefore, all profile data
# associated with this float were removed from subsequent analysis.
# Paulmier, A., D. Ruiz‐Pino, V. Garçon, and L. Farias (2006), 
# Maintaining of the eastern south Pacific oxygen minimum zone (OMZ) off Chile, Geophysical Research Letters, 33(20),
# doi:https://doi.org/10.1029/2006GL026801
opt_exclude_float <- '5905105'
...
if (opt_measure == 1){
  ...
  opt_xbreaks <- c(-2e-9, -1e-9, 0, 1e-9, 4e-9)
}
# Chl-a formatting
...
# join to bgc_fileid to retrive the file name for each profile
pH_anomaly_va <- left_join(pH_anomaly_va, bgc_fileid)

# Exclude the float opt_exclude_float
pH_anomaly_va <- pH_anomaly_va %>%
  filter(!grepl(opt_exclude_float, file)) %>%
  select(-c(file))


# select profiles of required depth
pH_anomaly_va <- pH_anomaly_va %>% 
  filter(profile_range %in% opt_profile_range &
           depth <= opt_depth_limit)
...
# # Mean and std deviation of anomaly
# anomaly_stats <- pH_anomaly_va_id %>%
#   mutate(
#     anomaly_mean = mean(anomaly, na.rm = TRUE),
#     anomaly_sd = sd(anomaly, na.rm = TRUE)
#   ) %>%
#   distinct ( anomaly_mean, anomaly_sd)
# 
# # Histogram of anomaly
# pH_anomaly_va_id %>%
#   ggplot(aes(anomaly)) +
#   geom_histogram(binwidth = 1E-11) +
#   geom_vline(xintercept = anomaly_stats$anomaly_mean, colour = "red") +
#   geom_vline(
#     xintercept = anomaly_stats$anomaly_mean - anomaly_stats$anomaly_sd,
#     colour = "red",
#     linetype = "dashed"
#   ) +
#   geom_vline(
...
#     linetype = "dashed"
#   ) +
#   labs(title = "anomaly distribution",
#        x = opt_measure_label,
#        y = "count")
# 
# # transform pH_anomaly_va_id by subtracting mean and dividing by std deviation
# pH_anomaly_va_norm <- pH_anomaly_va_id %>%
#   mutate (norm_anomaly = (anomaly-anomaly_stats$anomaly_mean) / anomaly_stats$anomaly_sd) %>%
#   select (file_id, depth, norm_anomaly)
# 
# # histogram or normalised anomaly
# pH_anomaly_va_norm %>%
#   ggplot(aes(norm_anomaly)) +
...
#     linetype = "dashed"
#   ) +
#   labs(title = "normalised anomaly distribution",
#        x = opt_measure_label,
#        y = "count")
...
# pH_anomaly_va_wide_norm <- pH_anomaly_va_norm %>%
#   pivot_wider(names_from = depth, values_from = norm_anomaly)
...
# pH_anomaly_va_wide_norm <- drop_na(pH_anomaly_va_wide_norm)

# Table for cluster analysis
points <- pH_anomaly_va_wide %>%
        select(-c(file_id))

# points <- pH_anomaly_va_wide_norm %>%
#         select(-c(file_id))
...
if (opt_analysis_type == 1) {
  ...
  wss <- numeric(opt_n_clusters)
  for (i in 1:opt_n_clusters) {
    
    # Fit the model: km.out
    print(paste0('Preparing to carry out test cluster analysis with ', i , ' clusters'))
    set.seed(1)
    kclust <- kmeans(points, centers = i, iter.max = opt_max_iterations, nstart = opt_n_start)
    
    # Save the within cluster sum of squares
    wss[i] <- kclust$tot.withinss
...
if (opt_analysis_type == 2) {
  ...
  set.seed(1)
  kclust <- kmeans(points, centers = opt_num_clusters, iter.max=opt_max_iterations, nstart=opt_n_start)
  
  cluster_profile <- t(cl_out$centers)
  cluster_ids <- kclust$cluster
...
if (opt_analysis_type == 2) {
  ...
  chla_cluster_mean <- chla_cluster %>% 
      filter(cluster >= 1 & depth <= opt_depth_limit) %>%
      group_by(cluster, depth) %>% 
