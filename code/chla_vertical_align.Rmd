---
title: "Prepare chla data and vertically align to (pH) climatology"
...
bgc_data_chla <- full_join(bgc_metadata_select, bgc_data_chla) %>%
                          select(
                            file_id,
                            lat, 
                            lon, 
...
# ensure we have a depth, pres_adjusted and chla_adjusted for all rows in bgc_data_chla
bgc_data_chla <- bgc_data_chla %>%
                  filter(!is.na(depth) & !is.na(pres_adjusted) & !is.na(chla_adjusted))
...
bgc_data_chla <- full_join(bgc_data_chla, bgc_apply_limits) %>%
...
for (i in 2:nrow(opt_profile_depth_range)) {
  ...
  #                         filter(!is.na(min_depth))
  bgc_data_chla <- bgc_data_chla %>% rows_update(bgc_apply_limits, by = "file_id")

}

# Find the gaps within the profiles
profile_gaps <- bgc_data_chla %>%
  select(
    file_id,
    depth
    ) %>%
...
for (i_gap in opt_gap_limit) {
  ...
  bgc_data_chla <- full_join(bgc_data_chla, profile_gaps_remove) %>%
                                  filter(is.na(gap)) %>%
                                  select(-c(gap))
}
...
target_depth_grid <-
  expand_grid(target_depth_range,
              bgc_data_chla_clean %>% distinct(file_id, lat, lon, date, profile_range))
...
bgc_data_chla_extended <- full_join(bgc_data_chla_clean, target_depth_grid) %>% 
...
bgc_data_chla_interpolated <-
  ...
  mutate(chla_spline = spline(target_depth, chla,
                                method = "natural",
                                xout = target_depth)$y) %>%
  ungroup()
...
bgc_data_chla_interpolated_clean <- 
  inner_join(target_depth_grid, bgc_data_chla_interpolated)
...
bgc_data_chla_interpolated_clean <- bgc_data_chla_interpolated_clean %>%
  mutate(year = year(date),
         month = month(date),
         .after = date)

# clean up working tables
rm(bgc_data_chla_extended, bgc_data_chla_interpolated)
...
rm(bgc_data_chla_interpolated_clean, ucsd_clim)
...
chla_overall_mean_1 %>% 
  ...
  labs(title = paste0('Overall mean chlorophyll a to ', max_depth_1, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_overall_mean_2 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Overall mean chlorophyll a to ', max_depth_2, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_overall_mean_3 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Overall mean chlorophyll a to ', max_depth_3, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_year_mean_1 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean chlorophyll a to ', max_depth_1, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_year_mean_2 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean chlorophyll a to ', max_depth_2, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_year_mean_3 %>% 
  ...
  coord_cartesian(xlim = opt_xlim)+
  scale_x_continuous(breaks = opt_xbreaks)+
  labs(title = paste0('Yearly overall mean chlorophyll a to ', max_depth_3, 'm'), x=opt_measure_label, y='depth (m)')
...
chla_histogram %>% 
  ...
  labs(title = "chlorophyll a profiles per year and profile range", x = "year", y = "profile count", fill = "profile range")
