...
if (opt_norm_amolaly) {
  ...
  temp_anomaly_va_id <- left_join(temp_anomaly_va_id, surf_anomaly)
  
  temp_anomaly_va_id <- rbind(
  temp_anomaly_va_id %>%
    filter (abs_sa < 1.0),
  temp_anomaly_va_id %>% 
    filter (abs_sa >= 1.0) %>%
...
if (opt_analysis_type == 2) {
  ...
  for (inum_clusters in opt_num_clusters_min[opt_profile_range]:opt_num_clusters_max[opt_profile_range]) {    
    ...
    if (inum_clusters == opt_num_clusters_min[opt_profile_range]){
      temp_anomaly_cluster_all <- temp_anomaly_cluster %>%
        mutate(num_clusters = inum_clusters)
    } else {
      temp_anomaly_cluster_all <- rbind(temp_anomaly_cluster_all, temp_anomaly_cluster %>%
        mutate(num_clusters = inum_clusters))
    }
  }
  ...
  anomaly_cluster_mean <- temp_anomaly_cluster_all %>%
    group_by(num_clusters, cluster, depth) %>%
    summarise(
      count_cluster = n(),
  ...
  anomaly_cluster_mean_year <- temp_anomaly_cluster_all %>%
    group_by(num_clusters, cluster, depth, year) %>%
    summarise(
      count_cluster = n(),
      anomaly_mean = mean(anomaly, na.rm = TRUE),
      anomaly_sd = sd(anomaly, na.rm = TRUE)
  ...
  anomaly_year_mean <- temp_anomaly_cluster_all %>%
    group_by(num_clusters, cluster, year) %>%
    summarise(
      count_cluster = n(),
      anomaly_mean = mean(anomaly, na.rm = TRUE),
      anomaly_sd = sd(anomaly, na.rm = TRUE)
  ...
  cluster_by_year <- temp_anomaly_cluster_all %>% 
    count(num_clusters, file_id, cluster, year,
          name = "count_cluster")

  # Convert to profiles
  cluster_by_year <- cluster_by_year %>% 
    count(num_clusters, cluster, year,
          name = "count_cluster")
  
  # total of each type of cluster
  cluster_count <- cluster_by_year %>%
    group_by(num_clusters, cluster) %>% 
...
if (opt_analysis_type == 2) {
  ...
  # Count the measurements
  cluster_by_year <- temp_anomaly_cluster_all %>% 
    count(num_clusters, file_id, cluster, month,
          name = "count_cluster")

  # Convert to profiles
  cluster_by_year <- cluster_by_year %>% 
    count(num_clusters, cluster, month,
          name = "count_cluster")
...
if (opt_analysis_type == 2) {
  ...
  cluster_by_location <- temp_anomaly_cluster_all %>%
    count(num_clusters, file_id, lat, lon, cluster,
          name = "count_cluster")
...
cluster_by_year <- temp_anomaly_cluster_ext %>% 
  count(num_clusters, temp_extreme, temp_extreme_order, file_id, cluster, year,
        name = "count_cluster")
...
# Convert to profiles
cluster_by_year <- cluster_by_year %>% 
  count(num_clusters, temp_extreme, temp_extreme_order, cluster, year,
        name = "count_cluster")

# total of each type of cluster
cluster_count <- cluster_by_year %>%
  group_by(num_clusters, temp_extreme, temp_extreme_order, cluster) %>% 
  summarise(count_profiles = sum(count_cluster)) %>%
  ungroup()
...
# create figure of cluster mean profiles
anomaly_cluster_mean_ext %>%
  group_split(num_clusters, temp_extreme_order) %>%
  map(
    ~  ggplot(data = .x, ) +
      geom_path(aes(x = anomaly_mean,
                    y = depth)) +
...
# Count the measurements
cluster_by_year <- temp_anomaly_cluster_ext %>% 
  count(file_id, num_clusters, temp_extreme_order, temp_extreme, cluster, year,
        name = "count_cluster")
# Convert to profiles
cluster_by_year <- cluster_by_year %>% 
  count(num_clusters, temp_extreme_order, temp_extreme, cluster, year,
        name = "count_cluster")
...
# Count the measurements
cluster_by_year <- temp_anomaly_cluster_ext %>% 
  count(file_id, num_clusters, temp_extreme_order, temp_extreme, cluster, month,
        name = "count_cluster")
# Convert to profiles
cluster_by_year <- cluster_by_year %>% 
  count(num_clusters, temp_extreme_order, temp_extreme, cluster, month,
        name = "count_cluster")
...
cluster_by_location <- temp_anomaly_cluster_ext %>%
  count(num_clusters, temp_extreme_order, temp_extreme, file_id, lat, lon, cluster,
        name = "count_cluster")
