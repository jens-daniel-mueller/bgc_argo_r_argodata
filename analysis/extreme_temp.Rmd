...
# load in bgc-temperature data with profile QC flags of A and B, irrespective of whether a corresponding pH measurement exists 
argo_temp <- read_rds(file = paste0(path_argo_preprocessed, "/bgc_merge_temp_AB.rds"))
...
# Location of BGC-temperature profiles (with pH)
...
profile_temp_count_biome <- profile_temp_extreme %>% 
  distinct(season_order, season, biome_name, temp_extreme, platform_number, cycle_number) %>%
...
surface_temp_biome <- profile_temp_extreme %>% 
  filter(depth <= 20) %>% 
  group_by(season_order, season, biome_name, temp_extreme, platform_number, cycle_number) %>% 
  summarise(argo_surf_temp = mean(temp_adjusted, na.rm=TRUE),
            OceanSODA_surf_temp = mean(OceanSODA_temp, na.rm = TRUE))
...
profile_temp_count_basin <- profile_temp_extreme %>% 
  distinct(season_order, season, basin_AIP, temp_extreme, platform_number, cycle_number) %>% 
...
surface_temp_basin <- profile_temp_extreme %>% 
  filter(depth <= 20) %>% 
  group_by(season_order, season, basin_AIP, temp_extreme, platform_number, cycle_number) %>% 
...
```{r map_all_temp_profiles, fig.asp=2}
