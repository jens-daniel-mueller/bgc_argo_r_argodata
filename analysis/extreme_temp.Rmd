...
```{r biomes_flat_map_pre_grid_reduction, fig.asp=0.5}
...
```{r biomes_flat_map_post_grid_reduction, fig.asp=0.5}
...
```{r basin_flat_map_pre_grid_reduction, fig.asp=0.5}
map +
...
```{r basin_flat_map_post_grid_reduction, fig.asp=0.5}
...
```{r apply_region_masks_OceanSODA_temp, fig.asp=0.5}
...
OceanSODA_temp_2x2_SO %>%
  ...
  map(
    ~ map +
      geom_tile(data = .x,
                aes(lon_raw, lat_raw, fill = clim_diff)) +
...
```{r flat_map_OceanSODA_temp_extremes_grid, fig.asp=2, eval=FALSE}
...
```{r flat_map_temp_extremes_1x1_plot, fig.asp=2}
...
# 618 000 000 rows 
# actual number of rows: 1 616 384    
# (using temperature profile QC flag A only where the pH QC flag is A as well)
...
```{r select_specific_temp_profile_2016, eval=FALSE}
...
##### Atlantic basin, biome 2, winter

```{r temp_profile_season_biome_Atlantic}
...
##### Atlantic basin, biome 1, spring

```{r Atlantic_biome_1_spring_mean_profiles}
...
profile_temp_extreme_jan <- inner_join(
  full_argo %>%
    filter(month == 1) %>%
    select(
      c(
...
profile_temp_extreme_jan <- profile_temp_extreme_jan %>%
  unite('platform_cycle',
...
profile_temp_extreme_jan <- profile_temp_extreme_jan %>%
  mutate(
    ...
    depth_binned = as.numeric(as.character(depth_binned)))
...
boa_clim_argo_temp_jan_SO <- boa_clim_argo_temp_jan_SO %>%
  mutate(
    depth_binned = cut(
      ...
      include.lowest = TRUE,
      labels = as.factor(unique(profile_temp_extreme_jan$depth_binned))
    ),
    depth_binned = as.numeric(as.character(depth_binned))
  )
...
boa_clim_argo_temp_jan_SO_binned <- boa_clim_argo_temp_jan_SO %>% 
  group_by(lon, lat, depth_binned, month) %>% 
...
## Raw profiles - January

Points are the january climatological temperature, lines are the raw Argo profiles colored by H/N/L classification
...
```{r plot_january_anomaly_profiles}
...
remove_clim_jan %>% 
  ...
  map(
    ~ggplot(data = .x,
      ...
      scale_fill_manual(values = HNL_colors)+
      facet_grid(basin_AIP~biome)+
      labs(title = paste0('year:', unique(.x$year)))
  )

```

## Overall mean anomaly profiles - January

```{r plot_overall_mean_clim_removed_profiles}

remove_clim_jan_overall_mean <- remove_clim_jan %>% 
  group_by(temp_extreme, depth_binned) %>% 
...
## Biome-mean anomaly profiles - January

```{r plot_biome_mean_clim_removed_profiles}

remove_clim_jan_biome_mean <- remove_clim_jan %>% 
  group_by(temp_extreme, depth_binned, biome) %>% 
  summarise(temp_anomaly_mean = mean(argo_temp_anomaly, na.rm = TRUE),
            temp_anomaly_sd = sd(argo_temp_anomaly, na.rm = TRUE))
...
## Basin-mean anomaly profiles - January

```{r plot_basin_mean_anomaly_profiles}

remove_clim_jan_basin_mean <- remove_clim_jan %>% 
  group_by(basin_AIP, temp_extreme, depth_binned) %>% 
  summarise(temp_anomaly_mean = mean(argo_temp_anomaly, na.rm = TRUE),
            temp_anomaly_sd = sd(argo_temp_anomaly, na.rm = TRUE))
