...
remove_clim_jan <- remove_clim_jan %>%
  mutate(
    ...
    depth = as.numeric(as.character(depth)),
    depth_boa = Hmisc::cut2(
      depth_boa,
      cuts = c(10, 20, 30, 50, 70, 100, 300, 500, 800, 1000, 1500, 2000, 2500),
      m = 5,
      levels.mean = TRUE),
    depth_boa = as.numeric(as.character(depth))
    )

# all.equal(remove_clim_jan$depth, remove_clim_jan$depth_boa)
  
# calculate the difference between the climatological argo and in-situ argo for each depth level and grid cell 

remove_clim_jan_year <- remove_clim_jan %>% 
  group_by(lon, lat, depth, year) %>% 
  mutate(argo_temp_anomaly = temp_adjusted - argo_clim_temp_jan) %>% 
  ungroup()
  ...
  map(
    ~ggplot(data = .x,
            aes(x = argo_temp_anomaly,
                ...
                group = platform_cycle,
                col = temp_extreme))+
      geom_point(data = .x %>% filter(temp_extreme == 'N'),
                aes(x = argo_temp_anomaly,
                    y = depth,
                    group = platform_cycle,
                    col = temp_extreme,
                    fill = temp_extreme),
                size = 0.2)+
      geom_point(data = .x %>% filter(temp_extreme == 'H'| temp_extreme == 'L'),
                 aes(x = argo_temp_anomaly,
                     y = depth,
                     group = platform_cycle,
                     col = temp_extreme,
...
remove_clim_jan_overall_mean <- remove_clim_jan_mean %>% 
  group_by(temp_extreme, depth) %>% 
  summarise(temp_diff_mean = mean(argo_temp_diff, na.rm = TRUE),
            temp_diff_sd = sd(argo_temp_diff, na.rm = TRUE))

remove_clim_jan_overall_mean %>% 
  ggplot()+
  geom_path(aes(x = temp_diff_mean,
                y = depth,
                group = temp_extreme,
                col = temp_extreme))+
  geom_ribbon(aes(xmax = temp_diff_mean + temp_diff_sd,
                  xmin = temp_diff_mean - temp_diff_sd,
                  y = depth,
                  group = temp_extreme,
                  fill = temp_extreme),
              col = NA,
...
remove_clim_jan_biome_mean <- remove_clim_jan_mean %>% 
  group_by(temp_extreme, depth, biome) %>% 
  summarise(temp_diff_mean = mean(argo_temp_diff, na.rm = TRUE),
            temp_diff_sd = sd(argo_temp_diff, na.rm = TRUE))

remove_clim_jan_biome_mean %>% 
  ggplot(aes(x = temp_diff_mean,
                y = depth,
                group = temp_extreme,
                col = temp_extreme))+
  geom_path()+
...
remove_clim_jan_basin_mean <- remove_clim_jan_mean %>% 
