...
map +
  geom_tile(data = region_masks_all_2x2, 
            aes(x = lon, 
                ...
                fill = biome))+
  scale_fill_brewer(palette = 'Dark2')+
  labs(title = 'RECCAP biomes (pre-grid reduction)')
...
map+
  geom_tile(data = nm_biomes,
            aes(x = lon,
                ...
                fill = biome_name))+
  scale_fill_brewer(palette = 'Dark2')+
  labs(title = 'new Mayot biomes (pre-grid reduction)')
...
map+
  geom_tile(data = region_masks_all_2x2,
            aes(x = lon,
                ...
                fill = biome))+
  scale_fill_brewer(palette = 'Dark2')+
  labs(title = 'RECCAP biomes post-grid reduction')
...
map+
  geom_tile(data = nm_biomes_2x2,
            aes(x = lon,
                ...
                fill = biome_name))+
  scale_fill_brewer(palette = 'Dark2')+
  labs('new Mayot biomes post-grid reduction')
...
OceanSODA_temp_2x2_SO <- inner_join(OceanSODA_temp_2x2_SO, nm_biomes_2x2 %>% 
                                      select(-n)) %>% 
  rename(new_biomes = biome_name)
...
# same plot, but with the new biomes:

OceanSODA_temp_SO_extreme_grid %>% 
  group_by(year, new_biomes, basin_AIP, temp_extreme) %>% 
  summarise(temp_regional = mean(temperature, na.rm = TRUE)) %>% 
  ungroup() %>% 
  ggplot(aes(x = year, y = temp_regional, col = temp_extreme))+
  geom_point(size = 0.3)+
  geom_line()+
  scale_color_manual(values = HNL_colors) +
  facet_grid(basin_AIP~new_biomes)+
  theme(legend.position = 'bottom')
```
...
# add in new Mayot southern Ocean biomes 
full_argo_2x2_SO <- inner_join(full_argo_2x2_SO, nm_biomes_2x2)
```
...
```{r temp_profiles_raw_RECCAP_biomes}
...
profile_temp_extreme %>%
  group_split(new_biomes, basin_AIP, year) %>% 
  # head(1) %>%
  map(
    ~ ggplot() +
      geom_path(data = .x %>% filter(temp_extreme == 'N'),
...
### Season x Mayot biomes

```{r mean_temp_profiles_season_new_biomes, fig.asp = 1}

profile_temp_extreme_biome <- profile_temp_extreme %>% 
  group_by(season, new_biomes, temp_extreme, depth) %>% 
  summarise(temp_biome = mean(temp_adjusted, na.rm = TRUE),
            temp_std_biome = sd(temp_adjusted, na.rm = TRUE)) %>% 
  ungroup()
...
profile_temp_count_biome <- profile_temp_extreme %>% 
  distinct(season, new_biomes, temp_extreme, platform_number, cycle_number) %>%
  group_by(season, new_biomes, temp_extreme) %>% 
  count(temp_extreme)
...
surface_temp_biome <- profile_temp_extreme %>% 
  filter(depth <= 20) %>% 
  group_by(season, new_biomes, temp_extreme, platform_number, cycle_number) %>% 
  summarise(argo_surf_temp = mean(temp_adjusted, na.rm=TRUE),
            OceanSODA_surf_temp = mean(OceanSODA_temp, na.rm = TRUE))
...
profile_temp_extreme_season <- profile_temp_extreme %>%
  group_by(season, new_biomes, basin_AIP, temp_extreme, depth) %>%
  summarise(temp_mean = mean(temp_adjusted, na.rm = TRUE),
            temp_std = sd(temp_adjusted, na.rm = TRUE)) %>%
  ungroup()
...
### RECCAP Biome-basin January mean profiles
...
profile_temp_extreme_biome_basin_jan %>%
  ...
  scale_color_manual(values = HNL_colors) +
  scale_fill_manual(values = HNL_colors)+
  labs(title = "Basin-RECCAP biome-mean January profiles",
       col = 'OceanSODA\ntemp anomaly \n(mean ± st dev)',
       fill = 'OceanSODA\ntemp anomaly \n(mean ± st dev)',
       y = 'depth (m)',
       x = 'mean Argo temperature (ºC)') +
...
```{r new_biome_basin_january_mean_profiles, fig.asp=1}

profile_temp_extreme_biome_basin_jan <- profile_temp_extreme %>%
  filter(month == 1) %>% 
  group_by(new_biomes, basin_AIP, temp_extreme, depth) %>%
  summarise(temp_mean = mean(temp_adjusted, na.rm = TRUE),
            temp_std = sd(temp_adjusted, na.rm = TRUE)) %>%
  ungroup()
...
profile_temp_extreme_biome_basin_jan %>%
  ...
  geom_path(aes(x = temp_mean,
                col = temp_extreme))+
  facet_grid(new_biomes~basin_AIP)+
  scale_color_manual(values = HNL_colors) +
  scale_fill_manual(values = HNL_colors)+
