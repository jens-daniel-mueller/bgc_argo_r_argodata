...
### Climatological monthly temperature
...
# profile_temp_extreme_jan <- profile_temp_extreme_jan %>% 
#   group_by(lon, lat, depth_binned, year) %>% 
#   mutate(temp_adjusted_binned = mean(temp_adjusted, na.rm = TRUE),
#          .after = temp_adjusted) %>% 
#   ungroup()
...
boa_clim_temp_jan_SO <- boa_clim_temp_jan_SO %>% 
  mutate(depth_binned = cut(depth_boa, 
                     breaks = c(10, 20, 30, 50, 70, 100, 300, 500, 800, 1000, 1500, 2000, 2500),
                     labels = as.factor(unique(profile_temp_extreme_jan$depth_binned))),
         depth_binned = as.numeric(as.character(depth_binned))) %>% 
  mutate(depth_binned = replace_na(depth_binned, min(profile_temp_extreme_jan$depth_binned)))


# remove_clim_jan <- inner_join(profile_temp_extreme_jan %>% 
#                                rename(OceanSODA_clim_temp = clim_temp,
#                                       OceanSODA_clim_diff = clim_diff),
#                              boa_clim_temp_jan_SO %>% 
#                                rename(argo_clim_temp_jan = clim_temp_jan) %>% 
#                                select(-time))

# bin depths in profile_temp_extreme_jan:
...
# calculate average climatological temp in each depth bin 

remove_clim_jan <- remove_clim_jan %>% 
  group_by(lon, lat, depth_binned) %>% 
  mutate(clim_temp_jan_binned = mean(argo_clim_temp_jan, na.rm = TRUE),
         .after = argo_clim_temp_jan) %>% 
  ungroup()
...
# remove_clim_jan_year <- remove_clim_jan %>% 
...
# remove_clim_jan_mean <- remove_clim_jan %>% 
#   group_by(lon, lat, depth) %>% 
#   mutate(argo_temp_diff = temp_adjusted - argo_clim_temp_jan) %>% 
#   ungroup()
