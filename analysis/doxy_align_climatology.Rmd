---
title: "Prepare dissolved oxygen data and vertically align to climatology"
...
This markdown file reads previously created dissolved oxygen climatology file and uses that as the definition of the depth levels that the BGC Argo dissolved oxygen data should be aligned to. Previously created BGC data (bgc_data.rds) and metadata (bgc_metadata.rds) are loaded from the BGC preprocessed folder.
...
The float dissolved oxygen profiles are then aligned using the spline function to match the depth levels of the climatology resulting in data frame bgc_data_doxy_interpolated_clean. An anomaly file is then created from the vertically aligned profiles and climatology.

Dependencies
------------

woa_doxy_clim.rds - created by load_argo_clim_doxy_woa
...
doxy_bgc_va.rds – vertically aligned dissolved oxygen profiles. 

doxy_clim_va.rds – climatology dissolved oxygen profiles. 

doxy_anomaly_va.rds – anomaly dissolved oxygen profiles. 
...
# The profile must have at least one dissolved oxygen reading at a depth <= opt_profile_depth_range[1, ]
# The profile must have at least one dissolved oxygen reading at a depth >= opt_profile_depth_range[2, ].
...
read dissolved oxygen climatology and prepare to a set range of depths

```{r read_climatology}

# climatology values (clim_doxy (°C)) available for lat, lon, month and pressure
woa_doxy_clim <- read_rds(file = paste0(path_argo_preprocessed, '/woa_doxy_clim.rds'))
...
target_depth_levels <- woa_doxy_clim %>% distinct(depth)
...
woa_doxy_clim_clean <- woa_doxy_clim %>%
                          select(
                            lat,
                            lon,
                            month,
...
woa_doxy_clim_clean <- woa_doxy_clim_clean %>% 
...
target_depth_grid <-
  expand_grid(target_depth_levels,
              woa_doxy_clim_clean %>% distinct(lat, lon, month))

# extend climatology depth vectors with target depths
woa_doxy_clim_extended <-
  full_join(woa_doxy_clim_clean, target_depth_grid) %>% 
...
woa_doxy_clim_interpolated <-
  woa_doxy_clim_extended %>%
  group_by(lat, lon, month) %>%
  mutate(clim_doxy_spline = spline(target_depth, clim_doxy,
                                       method = "natural",
                                       xout = target_depth)$y) %>%
  ungroup()

# subset interpolated values on traget depth grid
woa_doxy_clim_interpolated_clean <- 
  inner_join(target_depth_levels, woa_doxy_clim_interpolated)
...
woa_doxy_clim_interpolated_clean <-
  ...
  select(lat, lon, month,
         depth = target_depth,
         clim_doxy = clim_doxy_spline)

# clean up working tables
rm(woa_doxy_clim_clean, woa_doxy_clim_extended, woa_doxy_clim_interpolated)
```

## read doxy data

read dissolved oxygen profile and carry out basic checks, good data.
...
# we only want pressure and dissolved oxygen data
# conditions 
# n_prof == 1
# pres_adjusted_qc %in% c(1, 8) - pressure data marked as good
# doxy_adjusted_qc %in% c(1, 8) - dissolved oxygen data marked as good
# !is.na(pres_adjusted) - pressure value must be present
# !is.na(doxy_adjusted) - dissolved oxygen value must be present
...
#   labs(title = "dissolved oxygen profiles minimum depth",
#        x = "min depth (m)",
#        y = "profile count")
# 
# rm(out)
...
# For each profile what is the doxy at the shallowest depth of the profile, this is used in a comparison after vertical alignment
shallow_doxy_raw <- right_join(
  bgc_data_doxy,
  bgc_data_doxy %>%
    group_by(file_id) %>%
  ...
  select (file_id, doxy_shallow_raw = doxy_adjusted)
...
We have a set of dissolved oxygen profiles that match our criteria we now need to align that data set to match the 
depth that are in target_depth_levels, this will match the range of climatology values in woa_doxy_clim_interpolated_clean
...
# For each profile what is the doxy at the shallowest depth of the profile after vertical alignment
shallow_doxy_va <- right_join(
  bgc_data_doxy_interpolated_clean,
  bgc_data_doxy_interpolated_clean %>%
    group_by(file_id) %>%
    summarise(depth = min(depth)) %>% 
  ...
  select (file_id, doxy_shallow_va = doxy)

```

## Check shallow

A check is made that the shallowest doxy value after the vertical alignment is within a specified percentage of the shallowest doxy value before the vertical alignment
```{r check_shallow, eval=FALSE}

# Combined dataframe with shallow value pre and post vertical alignment
shallow_doxy_check <- 
  full_join(shallow_doxy_raw, shallow_doxy_va)

# Determine those profile that are outside of percentage criteria.
if (opt_shallow_check_perc > 0.0){
  shallow_check_report <- shallow_doxy_check %>%
    filter (abs((doxy_shallow_raw - doxy_shallow_va) / doxy_shallow_raw) > opt_shallow_check_perc)
...
if (opt_shallow_check_diff > 0.0){
  shallow_check_report <- shallow_doxy_check %>%
    filter (abs(doxy_shallow_raw - doxy_shallow_va) > opt_shallow_check_diff)
...
rm(shallow_doxy_check, shallow_doxy_raw, shallow_doxy_va, shallow_check_report)
gc()
...
# Create bgc_doxy_anomaly, but only where we have an climatology dissolved oxygen
bgc_doxy_anomaly <-
  inner_join(bgc_data_doxy_interpolated_clean, woa_doxy_clim_interpolated_clean)

# Calculate the anomaly dissolved oxygen
bgc_doxy_anomaly <- bgc_doxy_anomaly %>%
                      mutate(
                        anomaly = doxy - clim_doxy
                      )
...
# These profiles will be removed from both doxy_bgc_va and doxy_anomaly_va

# Anomaly max depth
bgc_doxy_anomaly_depth_check <- bgc_doxy_anomaly %>%
  group_by(file_id,
           profile_range) %>%
...
bgc_doxy_anomaly_depth_check <- left_join(bgc_doxy_anomaly_depth_check, opt_profile_depth_range)
...
remove_profiles <- bgc_doxy_anomaly_depth_check %>%
  filter((max_pdepth < max_depth) | (min_pdepth > min_depth)) %>%
  distinct(file_id)

# remove from both bgc_data_doxy_interpolated_clean and bgc_doxy_anomaly
bgc_data_doxy_interpolated_clean <-
  anti_join(bgc_data_doxy_interpolated_clean, remove_profiles)

bgc_doxy_anomaly <- anti_join(bgc_doxy_anomaly, remove_profiles)

```

## Write files

Write the climatology that maps onto depth levels, interpolated dissolved oxygen profiles that map onto depth levels and resulting anomaly files.
```{r write_doxy_va}
...
woa_doxy_clim_interpolated_clean %>%
  write_rds(file = paste0(path_argo_preprocessed, "/doxy_clim_va.rds"))

bgc_doxy_anomaly %>%
  write_rds(file = paste0(path_argo_preprocessed, "/doxy_anomaly_va.rds"))

# Rename so that names match if just reading existing files
doxy_bgc_va <- bgc_data_doxy_interpolated_clean
doxy_clim_va <- woa_doxy_clim_interpolated_clean
doxy_anomaly_va <- bgc_doxy_anomaly

#rm(bgc_data_doxy_interpolated_clean, woa_doxy_clim_interpolated_clean, bgc_doxy_anomaly)
...
doxy_clim_va <- read_rds(file = paste0(path_argo_preprocessed, "/doxy_clim_va.rds"))

doxy_anomaly_va <- read_rds(file = paste0(path_argo_preprocessed, "/doxy_anomaly_va.rds"))
...
# Profiles to 1200m
anomaly_overall_mean_2 <- doxy_anomaly_va %>% 
...
anomaly_year_mean_2 <- doxy_anomaly_va %>% 
  filter(profile_range %in% c(2, 3) & depth <= max_depth_2) %>%
  group_by(year, depth) %>% 
  summarise(doxy_count = n(),
            doxy_anomaly_mean = mean(anomaly, na.rm = TRUE),
...
# Profiles to 1500m
anomaly_overall_mean_3 <- doxy_anomaly_va %>% 
...
anomaly_year_mean_3 <- doxy_anomaly_va %>% 
  filter(profile_range %in% c(3) & depth <= max_depth_3) %>%
  group_by(year, depth) %>% 
  summarise(doxy_count = n(),
            doxy_anomaly_mean = mean(anomaly, na.rm = TRUE),
...
anomaly_overall_mean_1 %>% 
  ggplot()+
  geom_path(aes(x = doxy_anomaly_mean,
                y = depth))+
  geom_ribbon(aes(xmax = doxy_anomaly_mean + doxy_anomaly_sd,
                  xmin = doxy_anomaly_mean - doxy_anomaly_sd,
                  y = depth),
              alpha = 0.2)+
