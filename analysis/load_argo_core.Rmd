...
```{r QC_summary}
  ...
  core_profile_summary %>%
    group_split(measure_order) %>% 
    map(~ ggplot((.x), aes(x = year, y = count_profiles, col = profile_qc, group=profile_qc)) +
            geom_point() +
            geom_line() +
            facet_grid(. ~ measure) +
            labs(x = 'year', 
  ...
  core_measure_summary %>%
    group_split(measure_order) %>% 
    map(~ ggplot((.x), aes(x = year, y = count_measures, col = measure_qc, group=measure_qc)) +
            geom_point() +
            geom_line() +
            facet_grid(. ~ measure) +
            labs(x = 'year', 
