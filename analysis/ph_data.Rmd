...
region_masks_all <-
  stars::read_ncdf(paste(path_basin_mask, "RECCAP2_region_masks_all_v20210412.nc", sep = "")) %>%
  as_tibble() %>% 
  mutate(seamask = as.factor(seamask))
```

```{r separate_ocean_regions}
# harmonise the latitude longitude bands to the pH data (2x2 grid)
...
# add the region names to the surface pH data used for mapping 
...
Plot the difference between in-situ observed pH and the profile-mean surface pH for the upper 20 m

```{r SO_offset_from_the_mean}

# calculate the mean pH for each profile 
mean_profile_ph <- ph_surface_SO %>% 
  group_by(float_serial_no, cycle_number) %>% 
  mutate(mean_prof_ph = mean(ph_in_situ_total_adjusted), 
         .before = depth) %>% 
  ungroup() %>% 
...
```{r climatological_monthly_map, fig_asp=2}
...
basemap(limits = -32, data = ph_mean_SO) +   # change to polar projection 
...
Timeseries of monthly mean pH values, over the three different Southern Ocean regions
...
ggplot() +
  ...
  labs(title = 'Southern Ocean regions', 
       fill = 'region')
```
...
Plot the monthly average pH, per year (from Jan 2015 - Dec 2020), for each Southern Ocean region (1, 2, 3)
...
ph_month_SO %>%
  ...
  labs(x = 'month',
       y = 'pH in situ (total scale)',
       title = 'monthly mean pH (Jan 2015-Dec 2020, Southern Ocean)',
       col = 'year')
```
