...
Plot the difference between in-situ observed pH and the mean surface pH (20m) for each depth level

```{r SO_offset_from_the_mean}

ph_SO <- ph_surface_SO %>%
  group_by(month) %>%
  summarise(ph_month_ave = mean(ph_in_situ_total_adjusted))   # mean pH in top 20 m for each month

ph_SO = full_join(ph_SO, ph_surface_SO, by = 'month') %>%
  mutate(offset = ph_in_situ_total_adjusted-ph_month_ave, 
         .after = ph_month_ave) # subtract the surface mean from the in-situ pH, for each month

ph_SO %>%
  ggplot()+
  geom_jitter(aes(x = offset, y = depth, col = as.character(year)), size = 0.3, pch = 19) +
  scale_y_reverse()+
