...
# opt_analysis_type = 2 do cluster analysis and further analysis on the identified clusters
opt_analysis_type <- 2
...
# SO pH
# Add restrictions to ensure SO and there is also a pH profile
# read data
...
temp_anomaly_va <- inner_join(temp_anomaly_va, bgc_ph %>% distinct(file_id)) 
...
for (iprofile_type in 1:2) {
  ...
  for (i in 1:3) {
    ...
    # select profile based on extreme type and profile_type (base / adjusted)
    temp_anomaly_va_id <- temp_anomaly_va %>% 
      filter(temp_extreme == extreme_type[i] & profile_type == sel_profile_type)
