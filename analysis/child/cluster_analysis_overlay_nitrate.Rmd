...
### Overlay nitrate anomaly

for each cluster identified show with second attribute overlayed

```{r overlay_nitrate_prep}

# Read nitrate data
overlay_va <- read_rds(file = paste0(path_argo_preprocessed, "/nitrate_anomaly_va.rds")) %>%
  select (file_id,
          date,
          year,
          month,
...
overlay_color <- "#ED7D31"
opt_overlay_measure_label <- opt_nitrate_measure_label
