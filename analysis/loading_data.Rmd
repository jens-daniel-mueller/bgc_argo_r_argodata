...
ph_surface_1x1 <- full_join(bgc_data, bgc_metadata) %>%
  rename(lon = longitude, 
         lat = latitude) %>% 
  mutate(lon = if_else(lon < 20, lon + 360, lon)) %>% 
  mutate(
...
ph_surface_2x2 <- full_join(bgc_data, bgc_metadata) %>%
  rename(lon = longitude, 
         lat = latitude) %>% 
  mutate(lon = if_else(lon < 20, lon + 360, lon)) %>% 
  mutate(
    ...
    lat = as.numeric(as.character(lat)),
    lon = cut(lon, seq(20, 380, 2), seq(20.5, 379.5, 2)), 
    lon = as.numeric(as.character(lon))
   ) %>% 
  select(
    -c(doxy_adjusted:nitrate_adjusted_error),  # check column selection
    -c(profile_doxy_qc: profile_cdom_qc),
    -c(profile_cndc_qc:profile_up_radiance555_qc)
    ) %>% 
  mutate(depth = swDepth(pres_adjusted, latitude = lat),
