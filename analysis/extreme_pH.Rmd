...
# (adds in duplicate rows?
# OceanSODA_2x2 -> 1101584 rows
# OceanSODA_2x2_SO -> 3497664 rows)

# remove the duplicate rows created by inner_join 
OceanSODA_2x2_SO <- OceanSODA_2x2_SO %>% 
  distinct()
# (results in 248160 rows)
...
OceanSODA_regression <- OceanSODA_2x2_SO %>% 
  drop_na() %>% 
  nest(data = -c(lon, lat)) %>%   # group by lon, lat
  mutate(fit = map(.x = data,
                   .f = ~ lm(ph_month ~ date, data = .x)),
...
# rename date...25 = slope and date...3 = date
OceanSODA_2x2_SO_extreme_grid <- OceanSODA_2x2_SO_extreme_grid %>% 
  pivot_wider(names_from = term,
              values_from = estimate, 
              names_repair = 'unique') %>% 
  rename(date = date...3, 
         regression_slope = date...25,
...
```{r anomaly_test_plot_grid, fig.asp=0.5, eval=FALSE}
...
OceanSODA_2x2_SO_extreme_grid %>% 
  ...
  summarise(ph_regional = mean(ph_month, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
OceanSODA_2x2_SO_extreme_grid %>%
  ggplot(aes(ph_month, col = ph_extreme)) +
  geom_density() +
  scale_color_manual(values = HNL_colors) +
  facet_grid(basin_AIP ~ biome) +
...
OceanSODA_biome <- OceanSODA_2x2_SO %>% 
  ...
  summarise(biome_ph = mean(ph_month, na.rm = TRUE))
...
OceanSODA_biome_regression <- OceanSODA_biome %>% 
  drop_na() %>% 
  nest(data = -c(biome, basin_AIP)) %>%   # group by biome, basin
  mutate(fit = map(.x = data,
...
# pivot_wider two columns (slope and intercept), values_from = estimate, names_from = terms, names.repair = 'unique' 
# gives a slope and intercept column 
# rename date...26 = slope and date...3 = date
OceanSODA_2x2_SO_extreme_biome <- OceanSODA_2x2_SO_extreme_biome %>% 
  pivot_wider(names_from = term,
              values_from = estimate, 
              names_repair = 'unique') %>% 
  rename(date = date...3, 
         regression_slope = date...26,
         regression_intercept = `(Intercept)`)
```
...
profile_extreme_monthly <- profile_extreme %>% 
  ...
  summarise(ph_mean = mean(ph_adjusted_month, na.rm = TRUE),
            temp_mean = mean(temp_adjusted_month, na.rm = TRUE)) %>% 
  ungroup()
