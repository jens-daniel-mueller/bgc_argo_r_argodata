...
profile_count_biome <- profile_extreme %>% 
  distinct(season_order, season, biome_name, ph_extreme, platform_number, cycle_number) %>% 
...
surface_ph_biome <- profile_extreme %>% 
  filter(depth <= 20) %>% 
  group_by(season_order, season, biome_name, ph_extreme, platform_number, cycle_number) %>% 
  summarise(argo_surf_ph = mean(ph_in_situ_total_adjusted, na.rm=TRUE),
            OceanSODA_surf_ph = mean(OceanSODA_ph, na.rm = TRUE))
...
profile_count_basin <- profile_extreme %>% 
  distinct(season_order, season, basin_AIP, ph_extreme, platform_number, cycle_number) %>% 
...
surface_ph_basin <- profile_extreme %>% 
  filter(depth <= 20) %>% 
  group_by(season_order, season, basin_AIP, ph_extreme, platform_number, cycle_number) %>% 
