...
```{r january_clim_sst_ucsd, fig.asp=0.4}
...
clim_sst_ucsd %>% 
  ...
  map(
    ~map+
      ...
      scale_fill_viridis_c()+
      lims(y = c(-85, -28))+
      labs(title = 'Roemmich & Gilson UCSD clim SST, month:', unique(.x$month))
  )
...
```{r plot_climatological_jan_profiles_ucsd, fig.asp=1}
...
clim_argo_temp_ucsd %>% 
  ...
  map(
    ~ggplot(data = .x,
      ...
      scale_y_reverse()+
      facet_grid(basin_AIP~biome_name)+
      labs(title = 'UCSD clim profiles, month:', unique(.x$month))
  )
