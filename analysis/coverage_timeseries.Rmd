...
total_data_count <- bgc_metadata %>% 
  ...
  group_by(parameter) %>% 
  summarise(n = sum(n)) %>% 
  kable(., caption = 'total number of profiles (all QC flags)')
```


```{r count_usable_data, results='asis'}
...
usable_data_count <- bgc_metadata %>% 
  ...
  group_by(parameter) %>% 
  summarise(n = sum(n)) %>% 
  kable(., caption = 'total number of profiles with QC flags A, B, C, D, and E')
```


```{r count_A_flag_data, results='asis'}
...
A_data_count <- bgc_metadata %>% 
  ...
  group_by(parameter) %>% 
  summarise(n = sum(n)) %>% 
  kable(., caption = 'total number of profiles with QC flag A')
```


```{r count_F_flag_data, results='asis'}
...
F_data_count <- bgc_metadata %>% 
  ...
  group_by(parameter) %>% 
  summarise(n = sum(n)) %>% 
  kable(., caption = 'total number of profiles with QC flag F')
