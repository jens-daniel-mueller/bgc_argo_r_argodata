...
# lon <- ncvar_get(clim_argo_ph_jan, 'longitude')
# lat <- ncvar_get(clim_argo_ph_jan, 'latitude')
# depth <- ncvar_get(clim_argo_ph_jan, 'depth')
# time <- ncvar_get(clim_argo_ph_jan, 'time')
# pH <- ncvar_get(clim_argo_ph_jan, 'pH')

# range(lon)
# # range from 0.5 to 359.5
# range(lat)
# # range from -75 to -30 
# range(depth)
# # range from 2.1 to 2010.0 
# range(time)
# # 1 (january)
# range(pH, na.rm = TRUE)
# # 0.0000 to 8.190355 
# 
# lon = case_when(lon < 20 ~ lon +360,
#                 TRUE ~ lon)

clim_argo_ph_jan <- tidync::hyper_tibble(paste0(path_argo_clim_ph,
                                            "/PHclim_15March2022_Prelim.nc"))

# clim_argo_ph_jan <- stars::read_ncdf(paste0(path_argo_clim_ph,
#                                             "/PHclim_15March2022_Prelim.nc"))

# as_tibble(clim_argo_ph_jan)
