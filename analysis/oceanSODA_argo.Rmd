...
argo_OceanSODA_SO_clim_2x2 <- argo_OceanSODA_SO_1x1 %>% 
...
### Timeseries of Monthly Argo pH

```{r timeseries_argo_pH_per_month}
argo_OceanSODA_SO_1x1 %>%
  group_by(year, month, value) %>%  # compute regional mean Argo pH for the three biomes
  mutate(argo_ph_region = mean(argo_ph_month, na.rm = TRUE)) %>%
  ggplot(aes(x = year,
             y = argo_ph_region,
             col = value)) +
  facet_wrap(~month) +
...
```{r timeseries_argo_pH_per_region}
