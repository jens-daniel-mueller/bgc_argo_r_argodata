...
argo_OceanSODA_SO_lon_binned <- argo_OceanSODA_SO %>%
  mutate(lon = cut(lon, seq(20, 380, 20), seq(30, 370, 20)),
         lon = as.numeric(as.character(lon))
  ) %>%
  group_by(lon, year, month, value) %>%
  summarise(
    OceanSODA_ph_binned = mean(OceanSODA_ph, na.rm = TRUE),
    argo_ph_month_binned = mean(argo_ph_month, na.rm = TRUE)
  ) %>%
  ungroup()
