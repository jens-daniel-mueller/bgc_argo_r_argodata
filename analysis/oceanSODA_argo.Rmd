...
mean_clim_offset <- inner_join(argo_OceanSODA_SO_clim, region_masks_all_1x1_SO) %>% 
...
argo_OceanSODA_SO_clim_2x2 <- argo_OceanSODA_SO_clim_2x2 %>% 
  mutate(offset_clim_binned = 
           cut(offset_clim, 
               breaks = c(min(offset_clim, na.rm = TRUE), -0.025, -0.005, 0.000, 0.005, 0.025, 0.035, 0.05, max(offset_clim, na.rm = TRUE))),   # bin the offsets into intervals (create a discrete variable instead of continuous)
